    ...
    if r.status_code == 200:
        ...
        for row in csv.DictReader(f, skipinitialspace=True):
            ...
            if r.status_code == 200:
                ...
                LOG.warn(f"Failed to get imgqc results for session {session['ID']} - skipping")
        ...
        for test in tests:
            ...
            for test_def, test_results in zip(options.tests, options.test_results):
                ...
                if test["xnat_imgqc:name"] == test_name and test_is_relevant(img_name, matchers, exclusions, vendors):
                    LOG.info(f"Adding {test_name} result {test['xnat_imgqc:result']} for {img_name} to matcher: {matchers}, exclusions {exclusions}")
                    test_results.append(float(test["xnat_imgqc:result"]))
...
def main():
    ...
    try:
        ...
        for test_def, test_results in zip(options.tests, options.test_results):
            print(test_def, np.mean(test_results), np.std(test_results))
